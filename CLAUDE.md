...
완료: 작업 CRUD, 원고 생성(3엔진), 파생(스레드/인스타), 패턴 분석, 이미지 방식(실사/AI/혼합), 원고 편집

**다음 작업 (확정)**: 노출 가이드를 데이터로 관리하는 구조 —
(1) UI에서 편집 가능한 "공통 가이드" (알고리즘 변경 시 코드 수정 없이 갱신,
초기값은 blog-automation의 post-template.md 규칙에서),
(2) 패턴 "재분석" 버튼 (최신 상위 글을 다시 붙여넣어 가이드 갱신),
(3) 우선순위: 키워드 패턴 > 공통 가이드 > 기본 프롬프트.
그리고 blog-automation 저장소의 docs/블로그마케팅대행사-강의정리.md (유튜브 강의 노트) 학습해서 반영.
